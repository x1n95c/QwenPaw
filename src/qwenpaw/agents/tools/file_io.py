...
def _resolve_file_path(file_path: str) -> str:
    """Resolve file path: use absolute path as-is, resolve relative paths
    from the effective **project** directory.

    Relative paths intentionally resolve from the project dir, not the
    agent workspace, so a plain ``read_file("src/main.py")`` reaches the
    user's repository in every mode. ``get_tool_base_dir()`` falls back to
    the workspace when no project dir is configured, preserving the old
    behaviour for agents that never set one.
    ...
    if path.is_absolute():
        return str(path)
    return str(get_tool_base_dir() / file_path)
